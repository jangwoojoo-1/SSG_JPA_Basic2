...
import {useAccount} from "@/stores/account"; // ① 계정 스토어 객체 생성시 사용하는 함수 임포트
...
export default {
    get(url, params) { // ③ Axios 객체의 메서드 호출하여 HTTP GET 요청, 호출시 generateConfig메서드를 호출해서 인수로 입력한다.
        const config = generateConfig();
        config.params = params;
        return instance.get(url, config);
    },
    post(url, params) { // ④ Axios 객체의 메서드 호출하여 HTTP POST 요청, 호출시 generateConfig메서드를 호출해서 인수로 입력한다.
        return instance.post(url, params, generateConfig());
    },
    put(url, params) { // ⑤ Axios 객체의 메서드 호출하여 HTTP PUT 요청, 호출시 generateConfig메서드를 호출해서 인수로 입력한다.
        return instance.put(url, params, generateConfig());
    },
    delete(url) { // ⑥ Axios 객체의 메서드 호출하여 HTTP DELETE 요청, 호출시 generateConfig메서드를 호출해서 인수로 입력한다.
